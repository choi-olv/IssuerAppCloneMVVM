...
public class PayDetailRepository {
    ...
    // APIより支払明細の全データを再取得
    @SneakyThrows
    public LiveRealmData<PayDetail> getAll(Realm realm, Integer lastPostId, Integer postId) {

        // 前回実行時とパラメータが異なり、パラメータが有効(0以外)な場合のみAPIによるRealmの更新を行う
        if (lastPostId.intValue() != postId.intValue() && postId != 0) {
